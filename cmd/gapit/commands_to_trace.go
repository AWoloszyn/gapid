...
type commands2TraceVerb struct{ CommandsFlags }
...
func (verb *commands2TraceVerb) Run(ctx context.Context, flags flag.FlagSet) error {
	if flags.NArg() != 1 {
	...
	err = tct(ctx, client, tree.Root, func(n *service.CommandTreeNode, prefix string) error {
...
func tct(
	...
	if n.NumChildren > 0 && len(p.Indices) == 1 {
	...
	if n.Group != "" {
		...
		cmd, err := getCommand(ctx, c, n.Commands.First())
		if err != nil {
			return err
		}
		name = cmd.Name
	...
	fmt.Printf("AAA %v %v %v\n", p.Indices, tm, queueTime)
	fmt.Println(allEvents[len(allEvents)-1])
	...
	for i := uint64(0); i < n.NumChildren; i++ {
		err := tct(ctx, c, p.Child(i), f, prefix, i == n.NumChildren-1)
		if err != nil {
			return err
		}
	}
